...
export default function Additems({ setItems, items = [], itemTitle }) {
    ...
    <div className="sm:col-span-2">
      {showTag ? (
        ...
        <button
          ...
          type="button"
          className="flex items-center space-x-2 text-slate-800 dark:text-slate-50 py-2 px-4"
        >
          <Plus />
        ...
        {items.map((item, i) => {
          return (
            <div
              ...
              key={i}
              className="flex space-x-2 items-center bg-slate-600 px-4 py-2 rounded-lg cursor-pointer"
            >
              <p>{item}</p>
              <X className="w-4 h-4" />
            </div>
